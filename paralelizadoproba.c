...
//Inicializar el vector de personas con valores aletorios
	...
	while(cont<t_ejec && vivos != 0){
		...
		//Este array con tamaño normal, pero lleno solo una parte
		...
		// 		if(fabs(personaskter[i].posicion[0]-personas[j].posicion[0])<=pobl.radio_con && fabs(personaskter[i].posicion[1]-personas[j].posicion[1])<=pobl.radio_con){
		// 			if((personas[j].estado==1 || personas[j].estado==2) && personaskter[i].estado==0){
		// 				if(pobl.probabilidad_cont >= (rand()%101)) {
		// 					personaskter[i].estado = 1;
		// 				}
		// 			}
